      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">
            {accept ? `${accept} files` : "Any file"} up to{" "}
            {maxSize ? formatSize(maxSize) : "Unlimited"}
          </p>
        </div>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div
                    className={`p-2 rounded-lg ${item.status === "error" ? "bg-red-50" : "bg-gray-50"}`}
                  >
                    {item.status === "error" ? (
                    ...
                    {item.error ? (
                      ...
                      <div className="mt-2 w-full">
                        ...
                        <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-300 ${item.status === "complete" ? "bg-emerald-500" : "bg-blue-500"}`}
