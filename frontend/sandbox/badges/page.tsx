...
const BADGES: Badge[] = [
  {
    ...
    fullDescription: "Recognizes members who make productive use of workspace hours by checking in early and consistently.",
    earned: true,
    icon: "award",
  },
  {
    ...
    fullDescription: "Given to members who consistently honor reservations and maintain a reliable booking history.",
    earned: true,
    icon: "shield",
  },
  {
    ...
    fullDescription: "Celebrates collaborative behavior and positive contributions to the shared workspace community.",
    earned: false,
    icon: "star",
  },
  {
    ...
    fullDescription: "Highlights consistent workspace usage and planning discipline over a sustained period.",
    earned: false,
    icon: "award",
  },
];
...
export default function BadgeGalleryPage() {
  const { isLoading, canAccess } = useAuthRedirect({ requireAuth: true, redirectTo: "/login" });
  const [activeFilter, setActiveFilter] = useState<BadgeFilter>("all");
  const [selectedBadge, setSelectedBadge] = useState<Badge | null>(null);

  const filteredBadges = useMemo(() => {
    if (activeFilter === "earned") return BADGES.filter((badge) => badge.earned);
    if (activeFilter === "locked") return BADGES.filter((badge) => !badge.earned);
    return BADGES;
  }, [activeFilter]);
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 p-6">
      <section>
        <h1 className="text-2xl font-bold text-gray-900">Member Badge Gallery</h1>
        <p className="mt-1 text-sm text-gray-600">Track earned badges and view criteria for locked rewards.</p>
      </section>
      ...
      {activeFilter === "earned" && filteredBadges.length === 0 ? (
        <section className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center">
          <h2 className="text-lg font-semibold text-gray-900">No earned badges yet</h2>
          <p className="mt-2 text-sm text-gray-600">Complete bookings and engagement milestones to unlock your first badge.</p>
        </section>
      ) : (
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredBadges.map((badge) => (
            ...
            >
              ...
              <BadgeIcon icon={badge.icon} className="mb-3 h-7 w-7 text-gray-900" />
              ...
              <p className="mt-3 text-xs text-gray-500">
                {badge.dateEarned ? `Earned on ${badge.dateEarned}` : "Not earned yet"}
              </p>
            </button>
          ))}
        </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">{selectedBadge.name}</h2>
                <p className="text-xs text-gray-500">{selectedBadge.earned ? "Earned" : "Locked"}</p>
              </div>
              <button
                type="button"
            ...
            <p className="text-sm text-gray-700">{selectedBadge.fullDescription}</p>
            <div className="mt-4 rounded-lg bg-gray-50 p-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Earning Criteria</p>
              <p className="mt-1 text-sm text-gray-700">{selectedBadge.criteria}</p>
            </div>
