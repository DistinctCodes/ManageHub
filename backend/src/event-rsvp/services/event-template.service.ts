...
export class EventTemplateService {
  ...
  async createTemplate(
    createTemplateDto: CreateEventTemplateDto,
    ...
    try {
      ...
      this.logger.log(
        `Event template created successfully: ${savedTemplate.id}`,
      );
  ...
  async findAllTemplates(createdBy?: string): Promise<EventTemplate[]> {
    try {
      const where = createdBy
        ? { createdBy, status: TemplateStatus.ACTIVE }
  ...
  async updateTemplate(
    id: string,
    updateData: Partial<CreateEventTemplateDto>,
  ): Promise<EventTemplate> {
    try {
      ...
      const savedTemplate = await this.templateRepository.save(template);
      this.logger.log(
        `Event template updated successfully: ${savedTemplate.id}`,
      );
      return savedTemplate;
    } catch (error) {
    ...
    try {
      ...
      if (activeSeries > 0) {
        throw new BadRequestException(
          'Cannot delete template with active event series',
        );
      }
  ...
  async createEventFromTemplate(
    createFromTemplateDto: CreateEventFromTemplateDto,
  ): Promise<Event> {
    try {
      ...
      const eventDto = this.buildEventDtoFromTemplate(
        template,
      ...
      this.logger.log(
        `Event created from template ${template.id}: ${event.id}`,
      );
      return event;
    } catch (error) {
      this.logger.error(
        `Failed to create event from template: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async createEventSeries(
    createSeriesDto: CreateEventSeriesDto,
  ): Promise<EventSeries> {
    try {
      ...
      if (template.recurrenceType === RecurrenceType.NONE) {
        throw new BadRequestException(
          'Template must have recurrence configured to create a series',
        );
      }
      ...
      const series = this.seriesRepository.create({
        ...
        endDate: createSeriesDto.endDate
          ? new Date(createSeriesDto.endDate)
          : template.recurrenceEndDate,
        nextEventDate,
        eventsCreated: 0,
      });
  ...
  async resumeSeries(seriesId: string): Promise<EventSeries> {
    try {
      ...
      if (series.hasReachedLimit) {
        throw new BadRequestException(
          'Cannot resume series that has reached its limit',
        );
      }
      ...
      this.logger.error(
        `Failed to resume series ${seriesId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async cancelSeries(seriesId: string): Promise<EventSeries> {
    try {
      ...
      this.logger.error(
        `Failed to cancel series ${seriesId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private async createEventForSeries(
    series: EventSeries,
    eventDate: Date,
  ): Promise<Event> {
    try {
      ...
      this.logger.error(
        `Failed to create event for series ${series.id}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private buildEventDtoFromTemplate(
    ...
    return {
      ...
      organizerEmail:
        overrides.organizerEmail || template.defaultOrganizerEmail,
      requirements: overrides.requirements || template.requirements,
      agenda: overrides.agenda || template.agenda,
      price:
        overrides.price !== undefined ? overrides.price : template.defaultPrice,
      isPublic:
        overrides.isPublic !== undefined
          ? overrides.isPublic
          : template.isPublic,
      allowWaitlist:
        overrides.allowWaitlist !== undefined
          ? overrides.allowWaitlist
          : template.allowWaitlist,
      registrationDeadline: registrationDeadline.toISOString(),
      cancellationPolicy:
        overrides.cancellationPolicy || template.cancellationPolicy,
      additionalInfo:
        createDto.customMessage ||
  ...
  private calculateNextEventDate(
    currentDate: Date,
    template: EventTemplate,
  ): Date | null {
    if (template.recurrenceType === RecurrenceType.NONE) {
