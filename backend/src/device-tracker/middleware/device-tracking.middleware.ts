...
export class DeviceTrackingMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (deviceContext.shouldBlock) {
        this.logger.warn(`Blocking request from device ${deviceContext.deviceId} due to high risk score: ${deviceContext.riskScore}`);
        throw new ForbiddenException('Device access blocked due to security concerns');
      }
  ...
  private async processDeviceTracking(req: Request): Promise<DeviceContext> {
    ...
    let existingDevice = await this.findExistingDevice(deviceFingerprint, ipAddress, userId);
    ...
    if (existingDevice) {
      ...
      const updatedDevice = await this.geolocationService.updateDeviceGeolocation({
        ...existingDevice,
        lastSeenAt: new Date(),
        userAgent,
      });
      ...
      const newDeviceData = await this.geolocationService.updateDeviceGeolocation({
        userId,
        deviceType,
        ipAddress,
        userAgent,
      ...
      const newDevice = await this.deviceTrackerService.create(newDeviceData as any);
    ...
    if (existingDevice.riskScore !== riskAssessment.riskScore || 
        existingDevice.riskLevel !== riskAssessment.riskLevel) {
      await this.deviceTrackerService.update(deviceId, {
        riskScore: riskAssessment.riskScore,
        riskLevel: riskAssessment.riskLevel,
        status: riskAssessment.shouldBlock ? DeviceStatus.BLOCKED : DeviceStatus.ACTIVE,
      });
    }
