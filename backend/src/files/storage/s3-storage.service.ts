...
export class S3StorageService implements IStorageService {
  ...
  async getStream(fileId: string): Promise<Readable> {
    const res = await this.client.send(new GetObjectCommand({ Bucket: this.bucket, Key: fileId }));
    return res.Body as Readable;
  }

  async getMetadata(fileId: string): Promise<{ mimeType: string; originalName: string }> {
