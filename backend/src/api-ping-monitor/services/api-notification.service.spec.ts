...
describe('ApiNotificationService', () => {
  ...
  const createMockPingResult = (
    overrides: Partial<PingResult> = {},
  ): PingResult => ({
    id: 'result-1',
  ...
  const mockSuccessfulPingResult = createMockPingResult({
  ...
  describe('handlePingResult', () => {
    it('should handle successful ping result without alerts disabled', async () => {
      ...
      await service.handlePingResult(
        mockSuccessfulPingResult,
        endpointWithoutAlerts,
      );
    ...
    it('should check for slow response times', async () => {
      ...
      const checkSlowResponseSpy = jest
        .spyOn(service as any, 'checkSlowResponse')
        .mockResolvedValue(undefined);

      await service.handlePingResult(slowPingResult, mockEndpoint);

      expect(checkSlowResponseSpy).toHaveBeenCalledWith(
        mockEndpoint,
        slowPingResult,
      );
    });
    ...
    it('should check endpoint health', async () => {
      const checkEndpointHealthSpy = jest
        .spyOn(service as any, 'checkEndpointHealth')
        .mockResolvedValue(undefined);
    ...
    it('should handle errors gracefully', async () => {
      ...
      await expect(
        service.handlePingResult(mockSuccessfulPingResult, errorEndpoint),
  ...
  describe('checkSlowResponse', () => {
    ...
    it('should not send notification if no response time threshold is configured', async () => {
      const endpointWithoutThreshold = createMockEndpoint({
        alertConfig: {
          ...mockEndpoint.alertConfig,
          responseTimeThresholdMs: undefined,
        },
      });
      const sendNotificationSpy = jest
        .spyOn(service as any, 'sendNotification')
      ...
      await service['checkSlowResponse'](
        endpointWithoutThreshold,
        mockSuccessfulPingResult,
      );
  ...
  describe('calculateSeverity', () => {
    ...
    it('should return critical for many consecutive failures', () => {
      const result = service['calculateSeverity'](
        10,
  ...
  describe('isCooldownExpired', () => {
    ...
    it('should use custom cooldown period', () => {
      ...
      const result = service['isCooldownExpired'](
        'test-endpoint',
        2 * 60 * 1000,
      ); // 2 minutes
      expect(result).toBe(true);
  ...
  describe('testNotification', () => {
    ...
    it('should throw error if endpoint not found', async () => {
      ...
      await expect(
        service.testNotification('non-existent-id', 'email'),
  ...
  describe('updateNotificationSettings', () => {
    ...
    it('should throw error if endpoint not found', async () => {
      ...
      await expect(
        service.updateNotificationSettings('non-existent-id', {}),
      ).rejects.toThrow('Endpoint not found');
    });
  });
  ...
  describe('cleanup', () => {
    it('should remove old failure counters and notification times', () => {
      ...
      expect(service['lastNotificationTimes'].has('recent-endpoint')).toBe(
        true,
      );
  ...
  describe('sendNotification', () => {
    ...
    it('should send slack notification when configured', async () => {
      const sendSlackSpy = jest
        .spyOn(service as any, 'sendSlackNotification')
        .mockResolvedValue(undefined);
    ...
    it('should send webhook notification when configured', async () => {
      const sendWebhookSpy = jest
        .spyOn(service as any, 'sendWebhookNotification')
        .mockResolvedValue(undefined);
    ...
    it('should handle notification failures gracefully', async () => {
      ...
      jest
        .spyOn(service as any, 'sendSlackNotification')
        .mockRejectedValue(new Error('Slack failed'));
      jest
        .spyOn(service as any, 'sendWebhookNotification')
      ...
      await expect(
        service['sendNotification'](mockEvent),
      ).resolves.not.toThrow();
    });
  });
});
