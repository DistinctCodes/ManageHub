...
export class ApiPingMonitorController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid input or endpoint already exists',
  })
  ...
  @Get('endpoints')
  @ApiOperation({
    summary: 'Get all API endpoints with filtering and pagination',
  })
  @ApiResponse({ status: 200, description: 'Endpoints retrieved successfully' })
  async getEndpoints(@Query(ValidationPipe) queryDto: ApiEndpointQueryDto) {
  ...
  async bulkUpdateEndpoints(
    ...
    this.logger.log(
      `Bulk updating ${bulkUpdateDto.endpointIds.length} endpoints`,
    );
  ...
  async toggleEndpointActive(
    ...
    this.logger.log(
      `${body.isActive ? 'Activating' : 'Deactivating'} endpoint ${id}`,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'Provider endpoints retrieved successfully',
  })
  ...
  @ApiOperation({
    summary: 'Create preset endpoints for a provider (Stripe, Google, etc.)',
  })
  @ApiResponse({
    status: 201,
    description: 'Preset endpoints created successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Manual ping completed successfully',
  })
  async manualPing(
  ...
  async bulkPing(@Body(ValidationPipe) bulkPingDto: BulkPingDto) {
    this.logger.log(
      `Bulk ping requested for ${bulkPingDto.endpointIds.length} endpoints`,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'All active endpoints pinged successfully',
  })
  async pingAllActive(@Body() body: { triggeredBy: string }) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Ping results retrieved successfully',
  })
  async getPingResults(@Query(ValidationPipe) queryDto: PingResultQueryDto) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Ping result retrieved successfully',
    type: PingResult,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Endpoint ping results retrieved successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Endpoint history retrieved successfully',
  })
  @ApiQuery({
    name: 'days',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Uptime analytics retrieved successfully',
  })
  @ApiQuery({
    name: 'period',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Specific endpoint ID',
  })
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'SLA target percentage (default: 99.9)',
  })
  @ApiQuery({
    name: 'period',
    required: false,
  ...
  async getHealthOverview() {
    ...
    return {
      ...
      criticalEndpoints: unhealthy.filter((e) => e.currentStatus === 'down')
        .length,
      degradedEndpoints: unhealthy.filter((e) => e.currentStatus === 'degraded')
        .length,
    };
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Monitoring service stopped successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Monitoring service restarted successfully',
  })
  ...
  async exportResults(@Body(ValidationPipe) exportDto: ExportPingResultsDto) {
  ...
  async getUptimeReport(@Query('period') period: '24h' | '7d' | '30d' = '24h') {
