...
use crate::types::{UserRole, AccessControlConfig, ProposalAction};
...
fn test_initialize() {
    ...
    env.as_contract(&contract_id, || {
        assert_eq!(AccessControlModule::get_role(&env, admin.clone()), UserRole::Admin);
...
fn test_set_role_by_admin() {
    ...
    env.as_contract(&contract_id, || {
        let result = AccessControlModule::set_role(&env, admin.clone(), user1.clone(), UserRole::Member);
        assert!(result.is_ok());
...
fn test_set_role_by_non_admin_fails() {
    ...
    env.as_contract(&contract_id, || {
        let result = AccessControlModule::set_role(&env, user1.clone(), user2.clone(), UserRole::Member);
...
fn test_pause_unpause() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert!(AccessControlModule::set_role(&env, admin.clone(), user1.clone(), UserRole::Member).is_ok());
    });
}
...
fn test_transfer_admin() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(AccessControlModule::get_role(&env, user1.clone()), UserRole::Admin);
        assert!(AccessControlModule::is_admin(&env, user1.clone()));

        // New admin should be able to perform admin operations
        let user2 = Address::generate(&env);
        assert!(AccessControlModule::set_role(&env, user1.clone(), user2.clone(), UserRole::Member).is_ok());
    });
}
...
fn test_cannot_remove_main_admin_role() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(result.unwrap_err(), AccessControlError::RoleHierarchyViolation);
    });
}
...
fn test_config_management() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(stored_config.membership_token_contract, Some(membership_contract));
        assert_eq!(stored_config.require_membership_for_roles, true);
...
fn test_membership_token_insufficient_balance() {
    ...
    env.as_contract(&contract_id, || {
        ...
        let result = AccessControlModule::set_role(&env, admin.clone(), user1.clone(), UserRole::Member);
        assert_eq!(result.unwrap_err(), AccessControlError::InsufficientMembership);
    });
}
...
fn test_membership_not_required_for_guest_role() {
    ...
    env.as_contract(&contract_id, || {
        ...
        let result = AccessControlModule::set_role(&env, admin.clone(), user1.clone(), UserRole::Guest);
        assert!(result.is_ok());
...
fn test_admin_transfer_security() {
    ...
    env.as_contract(&contract_id, || {
        let result = AccessControlModule::propose_admin_transfer(&env, admin.clone(), admin.clone());
        ...
        assert_eq!(AccessControlModule::get_role(&env, user1.clone()), UserRole::Admin);
        assert_eq!(AccessControlModule::get_role(&env, admin.clone()), UserRole::Guest);
...
fn test_multisig_initialization() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(AccessControlModule::get_role(&env, admin1.clone()), UserRole::Admin);
        assert_eq!(AccessControlModule::get_role(&env, admin2.clone()), UserRole::Admin);
...
fn test_multisig_proposal_creation_and_approval() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(AccessControlModule::get_role(&env, user.clone()), UserRole::Member);
    });
}
...
fn test_insufficient_multisig_approvals() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(AccessControlModule::get_role(&env, user.clone()), UserRole::Guest);
