...
mod tests {
    ...
    #[test]
    fn valid_uuid_v4_variant_b() {
        assert!(is_valid_token_id("f47ac10b-58cc-4372-b567-0e02b2c3d479"));
    }
    ...
    fn rejects_malformed_string() {
        assert!(!is_valid_token_id("not-a-uuid-at-all"));
    }
    ...
    fn owner_does_not_match() {
        let registry = vec![("token-1".to_string(), "owner-a".to_string())];
    ...
    #[test]
    fn multiple_tokens_same_owner() {
        let registry = vec![
            ("token-1".to_string(), "owner-a".to_string()),
            ("token-2".to_string(), "owner-a".to_string()),
        ];
        assert!(is_token_owner("token-2", "owner-a", &registry));
    }
}
